...
function addCoinsToProfile(amount) {
    if (game.userProfile.ownedUpgrades.includes('double_coins')) {
        amount *= 2;
    }
    game.userProfile.totalCoins += amount;
    saveUserProfile();
}
...
function levelComplete() {
    ...
    // Calculate earned coins for profile
    let earnedCoins = 10 + (game.level * 5); // Base coins per level
    earnedCoins += Math.floor(timeBonus / 50); // Bonus for time
    earnedCoins += game.coinsCollected; // Collected coins count
    if (allCoinsBonus > 0) earnedCoins += 10; // Bonus for all coins

    // Add coins to profile
    addCoinsToProfile(earnedCoins);

    updateUI();

    if (game.level >= LEVELS.length) {
        // Game complete - save highscore only at the end
        earnedCoins += 50; // Bonus for completing the game
        addCoinsToProfile(50);
        saveHighscore();
        updateCoinsDisplay();
        ...
        document.getElementById('level-earned-coins').textContent = earnedCoins;
...
function init() {
    ...
    const lastPlayer = localStorage.getItem('littleBearLastPlayer') || 'Player';
    loadUserProfile(lastPlayer);
    loadHighscores();
    updateCoinsDisplay();

    // Set player name input from profile
    document.getElementById('player-name').value = game.userProfile.name;
    ...
    // Update coins display when player name changes
...
function startGame() {
    // Get player name from input and load their profile
    const nameInput = document.getElementById('player-name');
    const playerName = nameInput.value.trim() || 'Player';

    // Load profile for this player (creates new one if doesn't exist)
    loadUserProfile(playerName);
    saveUserProfile();
